...
app = Flask(__name__)
PHONE_NUMBER = "+918125193710"
MESSAGE_INTERVAL = 60
#GROUP_ID = "919398646386@c.us"

# Use local laptop webcam (device index 0) instead of Tapo IP camera
current_human_count = 0
total_detections = 0
start_time = time.time()
camera_index = 0  # 0 = default laptop webcam
camera_description = "Laptop Webcam (index 0)"
detection_active = True
FORCE_LOCAL_WEBCAM = True  # hint for code paths to prefer local webcam
...
def generate_live_frames_opencv():
    ...
    print(f"🔧 DEBUG: FORCE_LOCAL_WEBCAM = {FORCE_LOCAL_WEBCAM}")
    
    # Check if we should force local webcam usage
    if FORCE_LOCAL_WEBCAM:
        print("🎥 FORCE_LOCAL_WEBCAM enabled - using laptop webcam directly...")
        cap = cv2.VideoCapture(camera_index)  # Use the camera_index variable (0 for laptop)
        cap.set(cv2.CAP_PROP_BUFFERSIZE, 1)
        cap.set(cv2.CAP_PROP_FPS, 20)
        cap.set(cv2.CAP_PROP_FRAME_WIDTH, frame_width)
        cap.set(cv2.CAP_PROP_FRAME_HEIGHT, frame_height)
        if not cap.isOpened():
            print(f"❌ Failed to open laptop webcam at index {camera_index}.")
            return
        print(f"✅ {camera_description} opened at {frame_width}x{frame_height}.")
    else:
        # Try IP camera first, then fallback to local webcam
        rtsp_url = 'rtsp://NTLCAM:ntlcam123@192.168.0.218:554/stream2'
        print("🔎 Trying to open external IP camera...")
        ...
        if not cap.isOpened():
            print("⚠️  External camera not available. Falling back to local webcam (index 0)...")
            ...
            cap.set(cv2.CAP_PROP_FRAME_HEIGHT, frame_height)
            if not cap.isOpened():
                print("❌ Failed to open both external camera and local webcam.")
                return
            print(f"✅ Local webcam opened at {frame_width}x{frame_height}.")
        else:
            print(f"✅ External IP camera stream opened at {frame_width}x{frame_height}.")
...
def send_whatsapp_instant():
    ...
    print("📱 WhatsApp Web automation starting...")
    ...
    while True:
        try:
            ...
            msg = f"""🚨 CROWD UPDATE #{message_count}
👥 Humans: {current_human_count}
📊 Total: {total_detections}
⏰ {current_time.strftime('%H:%M:%S')}
🎯 Status: {'🟢 Active' if detection_active else '🔴 Inactive'}"""

            if not whatsapp_opened:
                # Only open WhatsApp Web ONCE at the beginning
                print("🌐 Opening WhatsApp Web for the first time...")
                encoded_msg = quote(msg)
                whatsapp_url = f"https://web.whatsapp.com/send?phone={"+918795932702"}&text={encoded_msg}"
                webbrowser.open(whatsapp_url)
                
                print("⏳ Waiting for WhatsApp Web to load...")
                time.sleep(12)  # Wait for page to load
                
                print("📤 Auto-sending first message...")
                pyautogui.press('enter')  # Send the message
                whatsapp_opened = True
                print("✅ WhatsApp Web opened and first message sent!")
                
            else:
                # For subsequent messages, use the same tab
                print(f"📤 Auto-sending message #{message_count} using existing tab...")
                
                try:
                    # Click on the text input area to focus
                    time.sleep(1)
                    pyautogui.click(960, 540)  # Click middle of screen to focus (adjust if needed)
                    time.sleep(0.5)
                    ...
                    pyautogui.write(msg, interval=0.01)  # Fast typing
                    time.sleep(0.5)
                    
                    # Send the message
                    pyautogui.press('enter')
                    
                    print(f"✅ Message #{message_count} auto-sent! Humans: {current_human_count}")
                    
                except Exception as auto_error:
                    print(f"⚠️ Auto-send failed: {auto_error}")
                    print("🔄 Trying URL method with same tab...")
                    
                    # Fallback: Use URL method but try to use same tab
                    try:
                        encoded_msg = quote(msg)
                        whatsapp_url = f"https://web.whatsapp.com/send?phone={PHONE_NUMBER}&text={encoded_msg}"
                        
                        # Try to refresh current tab instead of opening new one
                        pyautogui.hotkey('ctrl', 'l')  # Focus address bar
                        time.sleep(0.5)
                        pyautogui.write(whatsapp_url, interval=0.01)
                        pyautogui.press('enter')
                        ...
                        print(f"✅ Message #{message_count} sent via URL refresh!")
                        
                    except Exception as url_error:
                        print(f"❌ URL method failed: {url_error}")
                    
        except Exception as e:
            print(f"❌ Error sending message: {e}")
            # Don't reset whatsapp_opened to avoid opening new tabs
        ...
        # Wait before sending next message
        time.sleep(MESSAGE_INTERVAL)
#GROUP_NAME = "My Crowd Alerts"

#def send_whatsapp_periodically():
    #global current_human_count
    #while True:
      #  msg = f"⚠ Crowd Stats Update ⚠\nCurrent Humans: {current_human_count}"
       # try:
        ...
        #        wait_time=10,
         #       tab_close=False
         #   )
        #    print(f"✅ Message sent | Humans: {current_human_count}")
       # except Exception as e:
...
if __name__ == '__main__':
    ...
    # WhatsApp functionality temporarily disabled for testing
    # print("📱 IMPORTANT: Make sure WhatsApp Web is open in your browser!")
    # print("🔄 Starting WhatsApp messaging in 15 seconds...")
    # time.sleep(15)  # Give time to open WhatsApp Web if needed
    
    # Start WhatsApp messaging thread
    # whatsapp_thread = threading.Thread(target=send_whatsapp_instant, daemon=True)
    # whatsapp_thread.start()
    # print("✅ WhatsApp messaging started using existing tab")
